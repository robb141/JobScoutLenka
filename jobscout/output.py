...
from .dates import posted_label_sk, posted_sort_key
from .models import JobPosting


_FAVICON_SVG = (
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">'
    '<text y=".9em" font-size="90">\U0001f9ea</text></svg>'
)
FAVICON_HREF = "data:image/svg+xml," + quote(_FAVICON_SVG)

TITLE = "Lenka's Biochem Job Scout"
LOCATION_SUMMARY = "Bratislava, Vienna, and the Kittsee border area"
...
<body>
  <header>
    ...
    <div class="meta">
      ...
      Roles: {escape(roles)}.<br>
      Regions: {escape(LOCATION_SUMMARY)}.
    </div>
  </header>
  <div class="toolbar">
    <input id="search" type="search" placeholder="Filter company, title, region, description">
      ...
      <table id="jobs">
        <thead>
          <tr>
            ...
            <th>Region</th>
            <th>Location</th>
            <th data-sort-attr="posted">Zverejnené</th>
            <th>Description</th>
            <th>Source</th>
            <th>Query</th>
          </tr>
  ...
  <script>
    ...
    // Slovak collation: č sorts right after c, ch after h, etc.
...
def _html_row(job: JobPosting) -> str:
  ...
  <td>{escape(job.region_match)}</td>
  <td>{escape(job.location)}</td>
  <td>{escape(posted_label_sk(job.posted_date))}</td>
