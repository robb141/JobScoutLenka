...
class JobSource(ABC):
        ...
        self.session.headers.update(
            {
                ...
                "Accept-Language": "sk,de;q=0.9,en;q=0.8",
            }
        )
    ...
    # Client errors that will not fix themselves on a retry. 404 in particular
    # is how karriere.at reports "this search has no results".
    NON_RETRYABLE_STATUS = {400, 401, 403, 404, 405, 410}
    ...
    def _get(self, url: str, timeout: int = 30, attempts: int = 3) -> requests.Response:
        ...
        for attempt in range(attempts):
            ...
            if attempt < attempts - 1:
                delay = 2 * (attempt + 1)
                logger.warning("%s: retrying %s in %ss (%s)", self.name, url, delay, last_error)
                time.sleep(delay)
